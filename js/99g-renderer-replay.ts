...
function topoReplayStop() {
  ...
  _topoReplayHideBar();
  // Defensive: clear the overlay text content so a stale "(end)"
  // string from this run can't surface if some other code path
  // (tutorial overlay, etc.) shows the overlay element again
  // without going through _topoReplayRenderFrame first.
  const overlay = document.getElementById('topo-replay-overlay');
