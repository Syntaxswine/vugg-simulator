...
function event_jeffrey_mine_serpentinization_onset(c) {
  ...
  // (Ni-rich at ~2000 ppm in mantle peridotite) into the fluid.
  c.temperature = Math.max(340, c.temperature - 40);
  c.fluid.pH = Math.min(11.2, c.fluid.pH + 0.7);
  c.fluid.Mg = Math.min(280, c.fluid.Mg + 30);
  c.fluid.SiO2 = Math.min(280, c.fluid.SiO2 + 80);
  c.fluid.O2 = Math.max(0.05, c.fluid.O2 - 0.05);
  c.fluid.Ni = Math.min(150, c.fluid.Ni + 30);
  c.fluid.Fe = Math.min(70, c.fluid.Fe + 10);
  c.fluid.S = Math.max(0, c.fluid.S - 0.5);     // serpentinization consumes S to magnetite
  c.fluid.CO3 = Math.max(2, c.fluid.CO3 - 1);   // CO2 driven off in alkaline fluid
  c.flow_rate = 0.4;
  return `Serpentinization onset: T ${c.temperature.toFixed(0)}°C, pH ${c.fluid.pH.toFixed(1)} hyperalkaline. Mg + SiO2 released from olivine + pyroxene hydration; Ni (${c.fluid.Ni.toFixed(0)} ppm) + Fe mobilized. Chrysotile + brucite + magnetite + awaruite gates open.`;
}
...
function event_jeffrey_mine_dike_alteration(c) {
  ...
  // from chromite enables chrome varieties. The Manning & Bird 1990
  // rodingite-pyroxene framework applies.
  c.temperature = Math.max(300, c.temperature - 30);
  c.fluid.Ca = Math.min(450, c.fluid.Ca + 100);   // dike plagioclase Ca released
  c.fluid.Al = Math.min(50, c.fluid.Al + 20);     // dike plagioclase Al released
  c.fluid.SiO2 = Math.min(380, c.fluid.SiO2 + 60); // continued silica release
  c.fluid.Mg = Math.max(180, c.fluid.Mg - 30);    // partial Mg consumption by chrysotile
  c.fluid.Cr = Math.min(2.5, c.fluid.Cr + 1.2);   // trace Cr from chromite reaction
  c.fluid.Fe = Math.min(90, c.fluid.Fe + 15);     // continued Fe mobilization
  c.fluid.pH = Math.min(11.0, c.fluid.pH - 0.1);  // marginal pH drop from Ca-Al hydrolysis
  c.flow_rate = 0.35;
  return `Mafic dike alteration: T ${c.temperature.toFixed(0)}°C. Ca + Al + Si released from dike (Ca=${c.fluid.Ca.toFixed(0)}, Al=${c.fluid.Al.toFixed(0)}); trace Cr (${c.fluid.Cr.toFixed(1)} ppm) enables chrome-diopside + chromian grossular. Manning & Bird 1990 rodingite-pyroxene window opens.`;
}
...
function event_jeffrey_mine_mid_rodingite(c) {
  ...
  // background dike chalcopyrite enables CYPRINE — the world-reference
  // Jeffrey aesthetic per Bernardini 1981 figs 12-15.
  c.temperature = Math.max(260, c.temperature - 25);
  c.fluid.Cu = Math.min(4.0, c.fluid.Cu + 2.5);   // trace Cu for cyprine
  c.fluid.Ca = Math.max(380, c.fluid.Ca - 20);    // partial Ca consumption by grossular/diopside
  c.fluid.Al = Math.max(35, c.fluid.Al - 10);     // partial Al consumption by grossular
  c.fluid.SiO2 = Math.max(320, c.fluid.SiO2 - 20); // partial Si consumption
  c.fluid.pH = Math.min(10.8, c.fluid.pH - 0.1);
  c.flow_rate = 0.25;
  return `Mid-rodingite — vesuvianite stage: T ${c.temperature.toFixed(0)}°C. Cu trace (${c.fluid.Cu.toFixed(2)} ppm) drives CYPRINE sky-blue vesuvianite (Bernardini 1981 figs 12-15 world reference). Ca + Al + Si all still in window for vesuvianite + continued grossular/diopside.`;
}
...
function event_jeffrey_mine_late_ca_silicates(c) {
  ...
  // Wollastonite + prehnite fire alongside as the late-stage Ca-Si
  // + Ca-Al-Si endmembers.
  c.temperature = Math.max(230, c.temperature - 20);
  c.fluid.Na = Math.min(80, c.fluid.Na + 60);     // dike feldspar Na released for pectolite
  c.fluid.Fe = Math.min(70, c.fluid.Fe + 5);      // continued Fe trace for prehnite color
  c.fluid.Mg = Math.max(100, c.fluid.Mg - 30);    // continued Mg consumption
  c.fluid.pH = Math.min(10.5, c.fluid.pH - 0.2);  // slow pH drop
  c.flow_rate = 0.15;
  return `Late Ca-silicates: T ${c.temperature.toFixed(0)}°C. Na rises (${c.fluid.Na.toFixed(0)} ppm) for pectolite; Fe (${c.fluid.Fe.toFixed(0)} ppm) for prehnite green color. Pectolite spray-on-grossular + wollastonite + prehnite all gating.`;
}
...
function event_jeffrey_mine_terminal_datolite(c) {
  ...
  // most rodingite minerals so it accumulates in the residual fluid).
  ...
  c.fluid.B = Math.min(8.0, c.fluid.B + 7.0);     // trace B for datolite (1+ gate clears)
  c.fluid.Ca = Math.max(280, c.fluid.Ca - 30);    // continued Ca consumption
  c.fluid.SiO2 = Math.max(250, c.fluid.SiO2 - 20);
  c.fluid.pH = Math.min(10.2, c.fluid.pH - 0.2);
  c.flow_rate = 0.05;
  return `Terminal datolite: T ${c.temperature.toFixed(0)}°C. Trace B (${c.fluid.B.toFixed(1)} ppm) concentrated in residual fluid — datolite gemmy_vitreous_terminated on prehnite/wollastonite caps the Jeffrey rodingite paragenesis.`;
}
