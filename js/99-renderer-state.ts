...
function topoActiveSim() {
  // Return the simulator whose mode is currently on-screen. When the
  // user is at the title screen (`currentGameMode === null`) OR has
  // just entered a mode whose sim hasn't been built yet (e.g. Quick
  // Play before clicking Generate), return null — the topo will show
  // its placeholder.
  //
  // Earlier versions of this function had a "fall back to any sim at
  // all" tail after the mode-specific branches, intended as defensive
  // back-up. In practice it leaked state across modes: Home → Quick
  // Play would return the previous Fortress sim because randomSim was
  // null, painting stale fortress crystals on the Quick Play panel.
  // The mode-gating comment above describes the intent; the fallback
  // contradicted it. Removed in commit-after-58b31b4.
