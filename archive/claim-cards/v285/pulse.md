# CLAIM CARD — pulse  (v285, seed 42, 100 steps)
...
**Scenario spec hash:** a2d3a69b032407c2e3e29f1d8e93f16e7dcc424e926976a66c09f79d456c7886
**Archived strip SHA-256:** 29e0d8b490dc6900550cff59d3c2a208362999cff5c05d8751d3532195145726
...
**Deterministic headline (1):** quartz
**Deterministic accessories (1):** siderite — A plausible Fe-carbonate response to the generic mid-run fluid pulse; it is non-diagnostic because this scenario is a testing scaffold rather than a locality.
**Statistical (1):** rhodochrosite — A plausible Mn-carbonate response to the generic pulse-and-cooling sequence; observed in only part of the three-seed evidence panel.
...
| 3 | rhodochrosite | 72 | 1 | 0 | nucleation |
...
## Environment trajectory (first → last, [min,max])
  - T: 348.26089435471204 → 302.731119436541 °C  [235.87220512805368, 348.26089435471204] (raw_simulation_state)
  - pH: 6.5 → 6   [6, 6.5] (raw_simulation_state)
  - Eh: -200 → -200 mV  [-200, -200] (raw_simulation_state)
  - salinity: 5 → 5 psu  [5, 5] (raw_simulation_state)
  - O2: 0 → 0 ppm (mg/kg solvent)  [0, 0] (raw_simulation_state)
  - concentration: 1 → 1 ×  [1, 1] (raw_simulation_state)

## Saturation drivers
  - SI_calcite: -3.402 → -4.409  [-4.409, -3.402]
  - SI_aragonite: -3.465 → -4.472  [-4.472, -3.465]
  - SI_dolomite: -7.937 → -8  [-8, -7.937]
  - SI_HMC: -2.331 → -3.339  [-3.339, -2.331]
  - SI_siderite: -2.016 → -2.52  [-2.52, -2.016]
  - SI_selenite: -8 → -8  [-8, -8]
...
## Authored pressure/stress/phase context (claim, not run testimony)
  ...
  - Ksp pressure rule: reaction-specific SUPCRTBL delta-logK grid; bilinear only inside density and per-reaction temperature masks; no constant reaction-volume proxy
    - calcite: temperature-clamped-to-envelope; active=true; ΔlogK=0.997870332; Requested 350 C is outside this reaction's 10-90 C fitted envelope; the nearest 90 C SUPCRTBL pressure correction is held constant for a continuous, non-extrapolated boundary.
    - aragonite: temperature-clamped-to-envelope; active=true; ΔlogK=0.951925426; Requested 350 C is outside this reaction's 10-90 C fitted envelope; the nearest 90 C SUPCRTBL pressure correction is held constant for a continuous, non-extrapolated boundary.
    - dolomite: temperature-clamped-to-envelope; active=true; ΔlogK=2.865237358; Requested 350 C is outside this reaction's 10-250 C fitted envelope; the nearest 250 C SUPCRTBL pressure correction is held constant for a continuous, non-extrapolated boundary.
    - siderite: temperature-clamped-to-envelope; active=true; ΔlogK=1.13791134; Requested 350 C is outside this reaction's 10-200 C fitted envelope; the nearest 200 C SUPCRTBL pressure correction is held constant for a continuous, non-extrapolated boundary.
    - rhodochrosite: temperature-clamped-to-envelope; active=true; ΔlogK=1.088396986; Requested 350 C is outside this reaction's 10-200 C fitted envelope; the nearest 200 C SUPCRTBL pressure correction is held constant for a continuous, non-extrapolated boundary.
    - anhydrite: temperature-clamped-to-envelope; active=true; ΔlogK=1.944711404; Requested 350 C is outside this reaction's 10-300 C fitted envelope; the nearest 300 C SUPCRTBL pressure correction is held constant for a continuous, non-extrapolated boundary.
    - barite: temperature-clamped-to-envelope; active=true; ΔlogK=1.849833024; Requested 350 C is outside this reaction's 10-300 C fitted envelope; the nearest 300 C SUPCRTBL pressure correction is held constant for a continuous, non-extrapolated boundary.
    - celestine: temperature-clamped-to-envelope; active=true; ΔlogK=0.912523686; Requested 350 C is outside this reaction's 10-200 C fitted envelope; the nearest 200 C SUPCRTBL pressure correction is held constant for a continuous, non-extrapolated boundary.
...
**Source:** archived executed run state; not reconstructed from scenario definition
  ...
  - Temperature: 348.26089435471204 → 302.731119436541 °C [235.87220512805368, 348.26089435471204], n=100
  - Secure aragonite assessment: 0/100 executed steps; first={"boundary_kbar":3.1070299398706833,"secure_aragonite":false}, last={"boundary_kbar":2.7869097131129204,"secure_aragonite":false}
...
## Layer, solid-solution, competition, and habit testimony
  - Accepted layers: 40; formula-bearing=0; solid-solution=0; binding competition=0; reactive transformation etches=0; masked horizons=0.
  ...
  - Habit crystal 2: quartz; prismatic; extent=individual; forms=["m{100}","r{101}","z{011} dominant"]; size authority={"extent_kind":"individual","cap_cm":1200,"record_cm":600}; CDR=null.
  - Habit crystal 3: rhodochrosite; banding_agate; extent=individual; forms=["rhythmic Mn/Ca banding","agate-like layered cross-section"]; size authority={"extent_kind":"individual","cap_cm":25,"record_cm":12.5}; CDR=null.
