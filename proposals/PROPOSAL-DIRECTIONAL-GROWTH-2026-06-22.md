...
| **3 — Intrinsic polarity (the real polar tenants)** ✅ SHIPPED (wart-fix; see note) | `_polarAxis` for **hemimorphite (Imm2)**, then **tourmaline (3m)**, **wurtzite (6mm)**, **greenockite (6mm)** — all verified present (catalog audit DONE, see Provenance; quartz cleared). Several already declare hemimorphic habit variants that today render symmetrically — this makes them true. Stacks with Phase-2 occlusion. | js/99i (asymmetric +c/−c scale) | **AS SHIPPED: ALWAYS-ON for the 4 polar tenants (intrinsic — NOT scenario-opt-in; the "opts in" gating in this row was superseded at ship time). Render-only → byte-identical (tag never touches counts/sizes).** Specimen check per species. | 
