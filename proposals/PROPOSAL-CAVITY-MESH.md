...
|---------|--------|------|--------|-----------------|-----------------|------|-------|
| 1 | 4A + 4B | mesh.cells[] container + growth swap | landed 2026-05-11 | 93fcc2d | yes (aliasing) | low | cells[i].fluid === ring_fluids[r] by reference; _runEngineForCrystal reads cell.fluid via mesh.cellOf(crystal). 113 / 113 tests green. |
| 2 | 4C + 4D | Per-vertex Laplacian + propagate-global-delta | landed 2026-05-11 | (HEAD; previous = 93fcc2d) | yes (dedup-by-fluid-identity) | medium | `mesh.diffuse(rate, fieldNames, ringTemps)` + `mesh.propagateDelta(preFluid, fieldNames, equatorFluid)` added; `_diffuseRingState` and `_propagateGlobalDelta` delegate. Math equivalence proof: same-ring neighbors share an aliased fluid → contributions cancel → mesh-edge Laplacian reduces to ring-Laplacian. 119 / 119 tests green. |
...
| 4b | 4F + 4I | `wall.rings` becomes a view + Crystal legacy field drop | unstarted | — | yes | medium | wall.rings[r][c] either deletes or computes from mesh.cells[i]; Crystal's wall_ring_index / wall_center_cell finally drop (Phase 1's deprecation completes). |
