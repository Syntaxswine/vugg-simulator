...
| 4     | Tempo module unification                   | unstarted   | —                    | Replay timer (99g) and the three live-play tempo timers (Simulation in 91, Quick Play in 96, Fortress in 97 — all share the same per-step pacing formula now) extract into a `_topoTempoTimer` module that drives all four. Brings replay's scrub bar to initial play too. Lots of duplication to dedupe; ripe for a careful refactor. |
