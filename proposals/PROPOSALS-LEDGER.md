...
| 11 | **Redox-gate omission sweep — follow-up fixes** | ⏳ HELD — CHEMISTRY-bound | sphalerite/wurtzite ZnS gate. **2026-06-16, three probes:** nucleation-RNG isolated (v198) ≠ blocker; graduated competition RULED OUT (toggle → identical drop); growth keystone built+tested = only a SLICE (drop 60→43, reverted). The dominant residual is CHEMISTRY — gating drops mottramite *nucleation* 52→22 (σ via fluid trajectory), which no RNG isolation reaches. NEXT = trace the indirect Pb/Cu/V fluid pathway (likely a model artifact: ZnS reduced-zone vs mottramite oxidized-zone) OR accept-and-tune (~50%). [[project_vugg_redox_census]] |
