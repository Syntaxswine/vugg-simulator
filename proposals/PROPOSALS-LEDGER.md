...
| 8 | **Quartz morphology arc** (alpine-cleft) | ✅ SHIPPED v206 (2026-06-19) — the honest subset | Reshaped into the ALPINE-CLEFT arc: scouting (`644b267`) proved fenster CONTENT-blocked (quartz σ = silica-abundance → an occupied band mislabels slow pegmatites), so the fix was to build the content home. **Grimsel/Aar Swiss cleft SHIPPED v206**: SCEPTRE (resorption→renewal phantom boundary — `grow_quartz` dissolves at σ<1, so a seal corrodes the tip + the breach regenerates a wider cap on one crystal; EXTENT not rate is the metric; js/45 classifyQuartzSceptre + js/99i two-body render; `tools/quartz-sceptre-scan.mjs`), SMOKY/MORION (radiogenic-host γ-dose + Al, fleet-wide colour fix, Rossman 1994), TESSIN (steep-rhomb face form). Blocker en route was the dilute-broth correction (K/Na/Al down — the prior broth grew feldspar/albite monsters that enclosed the quartz). titanite (#13) was the prerequisite, v205. **NOT shipped (honest gaps):** FENSTER (no honest σ home — would confabulate; the content-block stands) + GWINDEL (needs a tectonic-shear field the sim lacks; would cannibalize the sceptre showcase). Master: `proposals/HANDOFF-ALPINE-CLEFT-2026-06-19.md` + `RESEARCH-quartz-morphology-2026-06-12.md` §6 (shipped footer). |
...
| 13 | **Mineral catalog orphans** — franklinite, staurolite, ~~titanite (=sphene)~~ | ◐ 2 remain | Round-5/6 metamorphic + zeolite cohort. ~~epidote~~ **✅ SHIPPED v196** (`a3c1cb5`) + anchor tormiq_alpine_cleft **v197** (`5043d57`). ~~stilbite~~ ~~heulandite~~ **✅ SHIPPED v200** (2026-06-17) — the Deccan Stage-II zeolite couple (see §G + #14). ~~titanite~~ **✅ SHIPPED v205** (`6f5627a`, 2026-06-19) — CaTiSiO₅ alpine-cleft Ti-nesosilicate, the first piece of the alpine-cleft arc (#8); fires tormiq/porphyry/jeffrey/sunnyside, upgrades the tormiq cleft. **franklinite + staurolite remain.** |
