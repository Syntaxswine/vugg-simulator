...
| machine-wide lease / queue | ✅ **atomic.** `%TEMP%/vugg-foreman/run-lease.d` — a *directory*, because `mkdir` is the atomic primitive and fails EEXIST on every platform we run. Deliberately outside any checkout: a lease under `.local-evidence/` cannot see a rival checkout's lease, and two working copies sharing one CPU is the exact collision. Holder identity is `(pid, startedIso)`; PIDs recycle, and a lease naming only a PID is one reuse from a stranger blocking every future run |
| atomic acquire / authenticated release | ✅ **repaired on review.** The first version read → decided-free → wrote: two agents in that window both read "free", both wrote, and the second silently displaced the first — *the race the lease exists to prevent, rebuilt inside the lease*. Release deleted unconditionally, so a displaced owner could delete a **successor's** claim. Now: `mkdir` to acquire, rename-aside compare-and-swap to take over a dead claim, an unguessable token per acquisition, and both heartbeat and release verify it. A refused run leaves no claim behind |
