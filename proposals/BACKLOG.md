...
> ## 🪨 CRYSTAL NEIGHBOURHOODS — **Phase 1 of the paragenetic atlas (2026-08-17)**
>
> Rock Bot's spec: an atlas answering "why might I find these minerals together on one specimen",
> built on the honest dependency rebuild (`420bf22`) rather than replacing it. Phase 0's edge schema
> already matched the data contract; Phase 1 adds the **mineral-neighbourhood plate**
> (`docs/crystal-neighborhoods.html`, generated by `tools/gen-crystal-neighborhood-page.mjs` from
> `tools/crystal-neighborhood-page.template.html`) — a centred species, first-degree neighbours
> clustered by relationship, ancestry left / descent right so the plate reads as time, six
> relationship filters, and an evidence drawer behind every line.
>
> **Two schema facts worth keeping.** (1) The census recorded a sighting *count*; a count is not a
> receipt. `substrate-escape-census.mjs` now emits one record per sighting carrying scenario, seed,
> `nucleation_step` and the position string, and `gen-crystal-flow-tree.mjs` threads them onto the
> edge as `evidence.runtime` — additively, so the existing flow and causation plates are untouched.
> (2) The engines name some pairings twice on adjacent lines under different `source_state`; the
> generator rightly keeps both, so the *page* merges by (from, to, relation) and shows both receipts
> behind one arrow, never two identical arrows.
>
> **The ten acceptance conditions are an instrument, not a checklist** —
> `tools/neighborhood-acceptance.mjs`. Phase 2 conditions are declared PENDING rather than silently
> omitted, so the count never implies coverage this build lacks. NOT wired into `npm run ci`; the
> atlas artefacts have never been gated there, and adding a gate is a separate decision.
>
> ## ⚗️ DIAGENETIC LOGIC GATES — **proposed, prototype runs — `PROPOSAL-DIAGENETIC-LOGIC-GATES.md`**
>
> The Professor's ask, Rock Bot's shape: a constraint solver over *events*, not more mineral-pair
> arrows. **The finding that decides the build: `data/minerals.json` already separates growth
> envelopes (`T_range_C`, `MINERAL_GATES`) from survival limits (`pH_dissolution_below`,
> `acid_dissolution`, `thermal_decomp_C`).** A solver reading `T_range_C` as a survival limit would
> produce confident wrong histories; the survival fields exist and are what the gates read.
> Coverage: 128 species with a fluid envelope, 131 thermal, **35 mute** — and mute must answer
> "I cannot say", never "it survived". Verdict ladder is Rock Bot's five, never yes/no; boundary
> ties return `ambiguous`. Quartz's own record (`pH<4` **and** `F≥20`, HF etching) vs calcite's
> (`pH<5.5`) resolves the Professor's specimen to `required` — by non-overlapping envelopes, not by
> a universal acid rule. D0 core prototyped; D1 observation vocabulary, D2 page, D3 literature
> envelopes ahead.
